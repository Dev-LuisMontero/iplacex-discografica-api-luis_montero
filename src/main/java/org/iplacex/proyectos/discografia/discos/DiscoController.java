...
public class DiscoController {
    @Autowired
    private IDiscoRepository discos;

    @Autowired
    private IArtistaRepository artistas;
    ...
    public ResponseEntity<Disco> HandlePostDiscoRequest(@RequestBody Disco disco) {
        if (disco.idArtista == null || !artistas.existsById(disco.idArtista)) {
            return new ResponseEntity<>(null, null, HttpStatus.NOT_FOUND);        
    }
    Disco creado = discos.insert(disco);
    return new ResponseEntity<>(creado, null, HttpStatus.CREATED); 
    }

    // GET /discos 
    @GetMapping(value = "/discos", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<Disco>> HandleGetDiscosRequest() {
        return new ResponseEntity<>(discos.findAll(), null, HttpStatus.OK);
    }

    // GET /disco/{id}
    @GetMapping(value = "/disco/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Disco> HandleGetDiscoRequest(@PathVariable("id") String id) {
        Optional<Disco> temp = discos.findById(id);
    ...
    // GET /artista/{id}/discos — usa el método custom con @Query
    ...
    public ResponseEntity<List<Disco>> HandleGetDiscosByArtistaRequest(@PathVariable("id") String id) {
        List<Disco> lista = discos.findDiscosByIdArtista(id);                  
